...
public class Principal {
	 ...
	 * introduzcas el nmero de la opcion que quieras elegir, con ese numero entrara en un case del switch
	 * @param args
	...
	public static void main(String[] args) throws FileNotFoundException, NoSuchElementException, InputMismatchException {
		...
		do {
			do {
				menu();
				opcion= elegirOpcion("Elija la opcion entre 0 y 4",0,4);
				opcionCorrecta=true;
				switch(opcion) {
				case 0:		
					exit=true;
				...
				case 2:
					...
					break; 
				case 3:
					Resolvedor r= new Resolvedor(pedirArchivo("Introduzca el nombre del archivo json sucesores", rutaGeneral),rutaGeneral);
					r.resolverLaberinto();
					break;
				case 4:
					generarInsertarNodos(rutaGeneral);
					break;
				}
	...
	public static void menu() {
		...
		System.out.println("4. Generar nodos aleatorios");
	}
	/**
	 * En este metodo se escogen celdas aleatorias del laberinto proporcionado gracias al m�todo resolverLaberinto
	 * y se insertan en la frontera para comprobar su correcto funcionamiento a la hora de ordenar los nodos
	 * @param rutaGeneral ruta donde se encuentran los archivos con los que trabajaremos
	 */
	public static void generarInsertarNodos(String rutaGeneral) {
		ArrayList<Celda>laberinto=new ArrayList<Celda>();
		Frontera fronteraNodos=new Frontera();
		Celda celdaAuxiliar;
		Nodo nodoAuxiliar;
		Estado estadoAuxiliar;
		Random random = new Random();
		Resolvedor r= new Resolvedor(pedirArchivo("Introduzca el nombre del archivo json sucesores", rutaGeneral),rutaGeneral);
		laberinto=r.resolverLaberinto();
		
		if(laberinto.size() != 0) {
			for(int numeroNodo=0; numeroNodo<10;numeroNodo++) {
				celdaAuxiliar=laberinto.get(random.nextInt(laberinto.size()-1));
				celdaAuxiliar.setValor(random.nextInt(16));
				estadoAuxiliar=new Estado("", celdaAuxiliar.getPosicion(),celdaAuxiliar.getValor());
				nodoAuxiliar=new Nodo(numeroNodo,estadoAuxiliar,celdaAuxiliar.getValor(),null,0,0,0);
				fronteraNodos.push(nodoAuxiliar);
				System.out.println(nodoAuxiliar.toString());
			}
			System.out.println("\n###-Frontera Ordenada-###\n");
			for(int numeroNodo=0; numeroNodo<10;numeroNodo++) {
				System.out.println(fronteraNodos.pop());
			}
		}
	}
	/**
	...
	public static void GenerarImagen(String rutaGeneral) {
		String ruta = pedirArchivo("Introduzca el nombre del archivo json que contiene el laberinto", rutaGeneral);
	...
	public static void generadorJson(List<Celda> celdas, int filas, int columnas, int max_n, int[][] mov, String[] id_mov, String ruta) {
		...
		JSONObject JSONSucesores = new JSONObject();
		...
		try{
			FileWriter file = new FileWriter(ruta.concat("sucesores_"+filas+"X"+columnas+"_maze.json"));
		...
		//Creamos y escribimos el archivo .json de sucesores y lo guardamos en la ruta que nos han pasado
		//preguntar como generar intial y objective
		do {
		...
		JSONSucesores.put("INITIAL","("+posInicial[0]+", "+posInicial[1]+")");
		JSONSucesores.put("OBJETIVE","("+posObjetivo[0]+", "+posObjetivo[1]+")");
		JSONSucesores.put("MAZE","sucesores_"+filas+"X"+columnas+"_maze.json");
		System.out.println("\nJson del laberinto creado con exito: sucesores_"+filas+"X"+columnas+"_maze.json");

		try{
			FileWriter file = new FileWriter(ruta.concat("sucesores_"+filas+"X"+columnas+".json"));
			file.write(JSONSucesores.toString());
			file.flush();
			file.close();
		}catch(Exception ex){
			System.out.println("Error: "+ex.toString());
		...
		Lector l = new Lector(ruta.concat("sucesores_"+filas+"X"+columnas+"_maze.json"));
		if(l.comprobarInconsistencias()==true) {
			l.realizarLectura();
		}
	 ...
	 * @param filas Nmero de filas que contiene el laberinto
	 * @param columnas Nmero de columnas que contiene el laberinto
	 ...
	 * @param mov Posibles movimientos que realizar (numrico)
	 * @param filas Nmero de filas que contiene el laberinto
	 * @param columnas Nmero de columnas que contiene el laberinto
	 ...
	 * @param mov Posibles movimientos que realizar (numerico)
	 * @param filas Numero de filas que contiene el laberinto
	 * @param columnas Numero de columnas que contiene el laberinto
	 * @param id_mov Posibles movimientos que realizar (textual)
	 * @param celdaSiguiente Celda despues de realizar el movimiento
	 ...
	 * @param columnas Numero de columnas que contiene el laberinto
	 * @param id_mov Posibles movimientos que realizar (textual)
	 * @return Celda Celda que se va a utilizar para establecer las coordenadas de la siguiente celda y para establecer el ultimo movimiento de la celda Actual
	 */
	 ...
	 * @param siguienteMovimiento Valor de la posicion X e Y que debemos aplicar a la celda actual
	 * @param filas Numero de filas que contiene el laberinto
	 * @param columnas Numero de columnas que contiene el laberinto
	 * @return boolean Especifica si la celda a la que nos queremos mover este dentro o fuera de los limites del laberinto
	 */
	 ...
	 * @param celdaSiguiente Celda despues de realizar el movimiento
	 * @param celdaActual Celda antes de realizar el movimiento
	 * @param CAMINO_PARCIAL Conjunto de celdas que forman un camino (no tiene por que estar completado)
	 ...
	 * @param CAMINO_PARCIAL Conjunto de celdas que forman un camino (no tiene por que estar completado)
	 * @return boolean Comprobacion de si finalmente se ha hecho un bucle o no
	 */
	 ...
	 * @param siguiente Celda despues de realizar el movimiento
	 */
	 ...
	 * @param CAMINO_PARCIAL Conjunto de celdas que forman un camino 
	 * @param celdasNoVisitadas Conjunto de las celdas que todavia no pertenecen a ningun camino 
	 * @return List<> Devuelve actualizadas las celdas que todavia no pertenecen a ningun camino 
	 */
	...
	public static String pedirArchivo(String mensaje, String rutaGeneral) {
		...
		while (ruta_valida == false) {
			...
			File carpeta = new File(rutaCompletaArchivo);
			if(carpeta.isFile()) {
				ruta_valida=true;
			}else {
